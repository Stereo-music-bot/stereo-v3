...
export default class SeekCommand extends BaseCommand {
  constructor() {
    super('rewind',  {
      ...
      ownerOnly: false,
      channelType: 'guild',
      description: (m: Message) => m.translate("commands.player_settings.rewind.description"),
      usage: (m: Message) => m.translate("commands.player_settings.rewind.usage"),
      timeout: 3e3,
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    if (isNaN(position) || position < 0)
      return message.channel.send(message.translate("commands.player_settings.rewind.incorrectValue", { redtick }));
